...
On tensor-core GPUs cuDNN runs conv kernels in NHWC (channels-last), so a plain
``[B, C, L]`` conv1d spends a large share of its time transposing tensors NCHW<->NHWC
around every convolution. Running the identical convolution as a channels-last conv2d
(a singleton height dim) lets cuDNN use its native NHWC kernels with no transpose — same
maths, markedly less memory traffic. This module reproduces ``JsonModel``'s temporal
forward that way; it is numerically equivalent (to float rounding) to running the stack
as conv1d.
"""
...
def is_channels_last_able(network: nn.Sequential) -> bool:
    """
    Whether ``network`` is a TemporalConv stack this module can run channels-last.

    Requires: one or more ``TemporalBlock`` layers, then ``AdaptiveAvgPool1d(1)``, then
    ``Flatten``, then a non-temporal head (Linear/ReLU/Dropout/LayerNorm). Anything else
    (LSTM/GRU, non-pooled head, ...) is not handled and the caller should fall back.
    """
...
def _causal_conv2d(conv: nn.Conv1d, x: torch.Tensor) -> torch.Tensor:
    # Run a causal Conv1d as a conv2d over [B, Cin, 1, L], reproducing its left-pad +
    # Chomp1d causal trim. conv.weight materialises the weight-norm parametrisation.
...
def _block(blk: TemporalBlock, x: torch.Tensor) -> torch.Tensor:
    # Mirror TemporalBlock.forward: net1 (conv/chomp/relu/drop), net2, residual, relu.
    ...
    res = x if down is None else F.conv2d(x, down.weight.unsqueeze(2), down.bias)
...
def channels_last_tcn_forward(
    ...
    """
    Run a channels-last-able TemporalConv ``network``: ``window`` [B, C, L] -> [B, out].
    """

    pi = _pool_index(network)
    x = window.unsqueeze(2).to(memory_format=torch.channels_last)  # [B, C, 1, L]
    for i in range(pi):
