...
def _identity_plant(const=0.5):
    # 1 axis, F=3 inputs [pos, vel, dac], A=1. Identity norm so
    # physical == normalised.
...
def test_constant_delta_accumulates_linearly():
    # With identity norm and delta-P=c, free-running position at
    # step k is (k+1)*c.
    c = 0.3
...
def test_scheduled_sampling_feeds_truth_forward():
    # ss_prob=1 anchors every fed position to truth, so the
    # prediction at step k is teacher[k-1] + c (and c at step 0).
    # Validates truth is fed, not the prediction.
    c = 0.2
    plant = _identity_plant(c)
    warmup = torch.zeros(1, 3, 4)
    dac = torch.zeros(1, 4, 1)
    teacher = torch.tensor([[10.0], [20.0], [30.0], [40.0]]).unsqueeze(
        0
    )  # [1, H, 1]
    out = plant.roll_forward(warmup, dac, 4, teacher_pos=teacher, ss_prob=1.0)[
        0, :, 0
    ]
...
def test_free_running_ignores_teacher():
    # ss_prob=0 must reproduce the plain free-running trajectory even
    # if teacher given.
    c = 0.25
    plant = _identity_plant(c)
    warmup, dac = torch.zeros(1, 3, 4), torch.zeros(1, 5, 1)
    teacher = torch.full((1, 5, 1), 99.0)
    out = plant.roll_forward(warmup, dac, 5, teacher_pos=teacher, ss_prob=0.0)[
        0, :, 0
    ]
    assert torch.allclose(
        out, torch.tensor([(k + 1) * c for k in range(5)]), atol=1e-6
    )
