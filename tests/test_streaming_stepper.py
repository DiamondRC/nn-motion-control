"""Streaming-TCN stepper: RF <= window, and streaming == windowed rollout in eval."""
...
STREAM_CFG = "examples/deltabot/configs/plant_stream.json"
...
def test_stream_model_builds_and_has_receptive_field_within_window():
    m = _stream_model()
    # 4 blocks, kernel 3, dilations 1/2/4/8; 2 convs/block -> RF = 1 + 4*(1+2+4+8).
    rf = 1 + 4 * (1 + 2 + 4 + 8)
    assert rf == 61 <= 64
...
def test_streaming_stepper_satisfies_protocol():
    assert isinstance(StreamingConvStepper(_stream_model()), RolloutStepper)


def test_reset_matches_windowed_forward_last_frame():
    m = _stream_model().eval()
    torch.manual_seed(0)
...
def _identity_stream_plant(model, kind):
    # Real 3-axis deltabot layout (9 features = 3 axes x [pos,vel,dac]); identity norm.
    layout = RolloutLayout.from_config(RunConfiguration(STREAM_CFG))
    ones, zeros = torch.ones(9), torch.zeros(9)
    in_stats = NormStats(mean=zeros, std=ones, normalizable=ones.bool())
    t_stats = NormStats(
        mean=torch.zeros(3), std=torch.ones(3), normalizable=torch.ones(3).bool()
    )
    return Plant(model, in_stats, t_stats, layout, device="cpu", rollout_kind=kind)


def test_streaming_rollout_matches_windowed_rollout_in_eval():
    # The whole point: under RF <= window + last-frame head, streaming the model one
    # frame at a time reproduces recomputing the full window each step (float rounding).
    m = _stream_model().eval()
    torch.manual_seed(1)
    warmup = torch.randn(3, 9, 64)
    dac = torch.randn(3, 12, 3)

    win = _identity_stream_plant(m, "windowed")
    stream = _identity_stream_plant(m, "streaming")
    with torch.no_grad():
...
def test_streaming_matches_windowed_with_avg_pool_readout():
    # Pooled readout must stream identically to the windowed forward: RF 29 <= 64-16, so
    # all 16 pooled frames are in-window and streaming reproduces it (float rounding).
    ...
    with torch.no_grad():
        # Windowed reference: slide a 64-window and re-run the full forward each step.
        got = [stepper.reset(window)]
        win = window
        want = [m(win)]
        for fr in frames:
...
def test_streaming_rollout_is_differentiable():
    # BPTT-correct: activations stay in-graph, so a loss on the rollout backprops.
    m = _stream_model().train()
    torch.manual_seed(2)
    warmup = torch.randn(2, 9, 64)
    dac = torch.zeros(2, 6, 3)
    plant = _identity_stream_plant(m, "streaming")
