"""Diagonal SSM: dual-form equivalence (parallel scan == recurrent step) + streaming."""
...
SSM_CFG = "examples/deltabot/configs/plant_ssm.json"
...
def test_recurrent_streaming_equals_full_sequence_scan():
    # Streaming the SSM frame by frame must equal running the full growing sequence at
    # once — the SSM carries all history, so no window-truncation (unlike a TCN).
    m = JsonModel(config=RunConfiguration(SSM_CFG)).eval()
    torch.manual_seed(3)
    ...
    with torch.no_grad():
        ...
        for fr in frames:
            ...
            seq = torch.cat([seq, fr.unsqueeze(-1)], dim=2)  # grow; keep all old frames
            want.append(m(seq))
    for g, w_ in zip(got, want, strict=True):
        assert torch.allclose(g, w_, atol=1e-4, rtol=1e-3)


def _identity_ssm_plant(model):
    layout = RolloutLayout.from_config(RunConfiguration(SSM_CFG))
    ...
    return Plant(model, ins, tgt, layout, device="cpu", rollout_kind="recurrent")


def test_ssm_rollout_runs_and_is_differentiable():
    m = JsonModel(config=RunConfiguration(SSM_CFG)).train()
    torch.manual_seed(4)
    warmup = torch.randn(2, 9, 64)
    dac = torch.zeros(2, 6, 3)
    plant = _identity_ssm_plant(m)
